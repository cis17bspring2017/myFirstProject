...
#include "lifeslice.h"
enum {DEAD=0, ALIVE=1};
static QSize boardSize(250, 250);//changed from 1024 x 768
...
void LifeMainWindow::on_actionPopulate_Randomly_triggered() {
    ...
    for (int i=0; i<2500; ++i) {//changed from 10000
...
void LifeMainWindow::calculate() {
    ...
    while (m_running) {
        ...
        for (int c=0; c<segments; ++c) {
            ...
            slices << slice;             //appends slice to slices which is a list of slice elements
            /* Add the pieces to a collection
